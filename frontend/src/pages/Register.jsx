...
import { MailIcon, LockIcon, UserIcon, EyeIcon, EyeOffIcon, ChefHat } from '../components/Icons.jsx'
...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    if (result.success) {
      toast.success('Akun berhasil dibuat! Selamat datang di CariMakan.')
    ...
    <div className="relative flex min-h-[calc(100vh-72px)] w-full items-center justify-center py-12 px-4 sm:px-6">
      ...
      <div className="relative z-10 w-full max-w-[420px] animate-scaleIn rounded-3xl bg-white/95 p-8 shadow-2xl backdrop-blur-xl sm:p-10 border border-white/20">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="font-display text-2xl font-bold tracking-tight text-char sm:text-3xl">Buat Akun Baru</h1>
          <p className="mt-2 text-[14px] text-char-soft">
            Daftar gratis untuk mulai mereview restoran.
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={() => toast.info('Fitur daftar dengan Google segera hadir!')}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1.5 block text-xs font-bold text-char-soft">Nama Lengkap</label>
            <div className="relative">
              <UserIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-char-faint" />
              <input
                type="text"
                required
                value={form.name}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold text-char-soft">Konfirmasi Password</label>
            <div className="relative">
              <LockIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-char-faint" />
              <input
                type={showPassword ? 'text' : 'password'}
        ...
        <p className="mt-6 text-center text-[14px] text-char-soft">
          Sudah punya akun?{' '}
          <Link to="/login" className="font-bold text-basil-600 transition-colors hover:text-basil-700 hover:underline">
            Masuk di sini
          </Link>
        </p>
      </div>
